...
public class SpatialMeteredWindowStore<K, V>
    extends WrappedStateStore<SpatialWindowStore<Bytes, byte[]>, Windowed<K>, V>
    implements SpatialWindowStore<K, V> {
  ...
  SpatialMeteredWindowStore(final SpatialWindowStore<Bytes, byte[]> inner,
  ...
  public void init(final ProcessorContext context,
    ...
    this.context =
        context instanceof InternalProcessorContext ? (InternalProcessorContext<?, ?>) context
            : null;
  ...
  private void registerMetrics() {
    ...
    flushSensor = StateStoreMetrics.flushSensor(taskId.toString(), metricsScope, name(),
        streamsMetrics);
    e2eLatencySensor = StateStoreMetrics.e2ELatencySensor(taskId.toString(), metricsScope, name(),
        streamsMetrics);
  }
    ...
    if (wrapped instanceof CachedStateStore) {
      return ((CachedStateStore<byte[], byte[]>) wrapped).setFlushListener(
          record -> listener.apply(
              record.withKey(
                      WindowKeySchema.fromStoreKey(record.key(), windowSizeMs, serdes.keyDeserializer(),
                          serdes.topic()))
                  .withValue(new Change<>(
                      record.value().newValue != null ? serdes.valueFrom(record.value().newValue)
                          : null,
                      record.value().oldValue != null ? serdes.valueFrom(record.value().oldValue)
                          : null
                  ))
          ),
          sendOldValues);
  ...
  @Override
  public void putSpatial(K key, V value, long windowStartTimestamp, Point point) {
    Objects.requireNonNull(key, "key cannot be null");
    Objects.requireNonNull(point, "point cannot be null");
    try {
      maybeMeasureLatency(
          () -> wrapped().putSpatial(keyBytes(key), serdes.rawValue(value), windowStartTimestamp,
              point),
          time,
          putSensor
      );
  ...
  @Override
  public WindowStoreIterator<V> fetchByPointRadius(K key, long timeFrom, long timeTo,
      Point point,
      double radiusMeters) {
    Objects.requireNonNull(point, "point cannot be null");
    return new MeteredWindowStoreIterator<>(
        wrapped().fetchByPointRadius(keyBytes(key), timeFrom, timeTo, point, radiusMeters),
        fetchSensor,
        streamsMetrics,
        serdes::valueFrom,
        time
